...
public:
	...
	void getAdaptiveBuffer(unsigned char*& buf, int& w, int& h);
	double aspectRatio();

	void traceImage(int w, int h);
	int aaImage();
	int adaptiveImage(double thresh);
	int adaptSample(double thresh, int depth, double minx, double miny, double maxx, double maxy, glm::dvec3 &color);
	bool checkRender();
	void waitRender();
...
private:
	...
	std::vector<unsigned char> adaptive_buffer;
